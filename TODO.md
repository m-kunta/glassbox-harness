...
- [x] Revisit the real replenishment agent's decision, evidence, citation, and alternatives mapping after the P1 seed suite establishes end-to-end behavior; refine the domain semantics from observed evaluation results rather than adding an evaluation-only shadow model.
...
- [x] During P1.3 authoring, inspect the initial do-nothing cases for evidence or alternatives added only to satisfy the uniform evaluation floor; strengthen real-agent reasoning requirements if padding appears, without category exemptions.
...
| 2026-09-15 | P1's live-provider smoke mode is an agent-owned, explicitly opted-in, five-case profile using the replenishment agent's existing provider/model configuration and credentials. It reports results and operational metrics, but remains outside `glassbox eval` and all CI gates. | A fixed representative profile bounds spend and makes coverage reviewable; reusing agent configuration prevents Glassbox from owning provider SDKs or secrets. Live nondeterministic behavior is diagnostic evidence, not deterministic merge policy. | P1.3 | Mohith Kunta |
| 2026-09-20 | P1's replenishment adapter reassembles persisted evidence fields by caller-defined `evidence_id` before evaluation, and the replenishment agent records provider-produced alternatives rather than priority-tier templates. All golden fixtures now explicitly supply their scripted alternatives; the four do-nothing cases each name a distinct, fact-based rejected escalation. | Storage represents an evidence citation group as multiple field rows, while `EvidenceRecord` and the uniform P1 evidence floor operate on citation groups. A tier template allowed alternatives to satisfy the gate without showing case-specific reasoning; preserving the agent result avoids an evaluation-only shadow model and lets a missing alternative fail evaluation visibly. | P1.1, P1.3 | Mohith Kunta |
